...
const SectionLink = ({ page }) => (
  <Link key={page.slug} className="section-link" activeClassName="active-section-link" to={page.slug}>
...
const ModuleNavbar = ( { pageGroup } ) => {
  ...
  return (
    <div className="navbar-module-container">
      ...
      {
        pageGroup.nodes.map(page => <SectionLink key={page.frontmatter.slug} page={page.frontmatter}/>)
      }
    </div>
  );
}
...
const Navbar = () => {
  ...
  <div className="body-navbar-inner">
    {data.allMarkdownRemark.group.map( (group, index) => <ModuleNavbar key={index} pageGroup={group} />)}
  </div>
  );
}
